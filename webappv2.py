...
import locale
import requests
import io # Neu: Für das Einlesen der heruntergeladenen Excel-Daten
from streamlit_quill import st_quill

# --- KONFIGURATION ---
st.set_page_config(page_title="mycareernow Planer", page_icon="📅", layout="wide")

# 1. GITHUB LINKS (HIER ANPASSEN!)
# Link zu den Texten (Raw)
GITHUB_TEXTS_URL = "https://raw.githubusercontent.com/mkleuser93/kurs-planer/refs/heads/main/modul_texte_backup.json"
# Link zur Excel-Datei (Raw) - Muss .csv oder .xlsx sein
GITHUB_EXCEL_URL = "https://raw.githubusercontent.com/mkleuser93/kurs-planer/refs/heads/main/kursdaten.xlsx" 
...
def init_texts_from_github():
    if not os.path.exists(TEXT_FILE):
        try:
            if "DEIN_USER" not in GITHUB_TEXTS_URL:
                response = requests.get(GITHUB_TEXTS_URL)
                if response.status_code == 200:
                    with open(TEXT_FILE, "w", encoding="utf-8") as f:
                        f.write(response.text)
        except Exception as e:
            print(f"GitHub Text Error: {e}")

init_texts_from_github()

# --- DATEN LADEN & CACHING (NEU) ---
@st.cache_data(ttl=3600) # Speichert die Daten für 1 Stunde im Cache -> Extrem schnell
def load_excel_data_from_github():
    if "DEIN_USER" in GITHUB_EXCEL_URL:
        return None, "GitHub URL noch nicht konfiguriert."
    
    try:
        response = requests.get(GITHUB_EXCEL_URL)
        response.raise_for_status()
        
        # Prüfung ob CSV oder Excel anhand der Endung
        if GITHUB_EXCEL_URL.endswith('.csv'):
            df = pd.read_csv(io.BytesIO(response.content), sep=';')
        else:
            df = pd.read_excel(io.BytesIO(response.content))
            
        # Datenbereinigung direkt beim Laden
        ...
        return None, f"Fehler beim Laden der Excel-Datei: {e}"
...
if not st.session_state.is_admin:
    ...
    # MANUELLER CACHE CLEAR (Wichtig, wenn neue Daten auf GitHub liegen)
    if st.sidebar.button("🔄 Neue Daten von GitHub laden"):
        st.cache_data.clear()
        if os.path.exists(TEXT_FILE): os.remove(TEXT_FILE)
        init_texts_from_github()
        st.sidebar.success("Cache geleert & neu geladen!")
        st.rerun()
        
    cur = load_texts()
    st.sidebar.download_button("⬇️ Backup Texte", json.dumps(cur, indent=4), "backup.json", "application/json")
    if u := st.sidebar.file_uploader("⬆️ Restore Texte", ["json"]): save_all_texts_from_upload(u); st.sidebar.success("OK")
...
# Daten von GitHub laden (oder Cache nutzen)
df, error_msg = load_excel_data_from_github()

if error_msg:
    st.error(error_msg)
    st.info("Bitte konfiguriere die 'GITHUB_EXCEL_URL' im Code.")
elif df is None:
